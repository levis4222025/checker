...
from Gates.ppC import paypal_charge1
...
'''
    ...
    ccc = None
    if len(message.text.split('pp ')) > 1 and message.text.split('pp ')[1].strip():
            ccc = message.text.split('pp ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await paypal_charge1(cc,mm,yy,cvv)
    brand , type, level, bank, country, emoji = await bin_data(cc[:6])
    rank = await get_user_rank(user_id)
    end_time = time.perf_counter()
    elapsed_time = f"{end_time - start_time:.2f}"
    if 'Charged $5' in msg or 'CVV2_FAILURE.' in msg or 'INSUFFICIENT_FUNDS.' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
<b>#PayPal_Commerce ($5) 🔥 [/pp]
- - - - - - - - - - - - - - - - - - - - - - - -</b>
