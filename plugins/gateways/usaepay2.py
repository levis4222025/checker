...
from Gates.usaepay2 import usaepay2
...
import re,time
@Client.on_message(filters.command("ue2" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('ue2') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('ue2 ')) > 1 and message.text.split('ue2 ')[1].strip():
            ccc = message.text.split('ue2 ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await usaepay2(cc,mm,yy,cvv)
    brand , type, level, bank, country, emoji = await bin_data(cc[:6])
    rank = await get_user_rank(user_id)
    end_time = time.perf_counter()
    elapsed_time = f"{end_time - start_time:.2f}"
    if 'Order placed successfully!' in msg or 'CV2 FAIL' in msg or 'AVS mismatch' in msg or 'CVV2 Declined' in msg or 'Insufficient funds' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
