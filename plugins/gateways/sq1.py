...
import re,time
@Client.on_message(filters.command("sq" , prefixes=["/", "."]))
async def chk(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('sq') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('sq ')) > 1 and message.text.split('sq ')[1].strip():
            ccc = message.text.split('sq ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    card = f"{cc}|{mm}|{yy}|{cvv}"
    msg = await square_auth(card)
    brand , type, level, bank, country, emoji = await bin_data(cc[:6])
    rank = await get_user_rank(user_id)
    end_time = time.perf_counter()
    elapsed_time = f"{end_time - start_time:.2f}"
    if 'Payment Method Added' in msg or 'Payment method successfully added.' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
        msg = 'Authorized'
    elif 'Duplicate card exists in the vault.' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
        msg = 'Authorized - Duplicate'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
<b>#Square_Auth 🔥 [/sq]
