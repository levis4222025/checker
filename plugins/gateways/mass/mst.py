...
from Gates.st import stripe_charge1  # Ensure this function is async and non-blocking
...
async def process_card(cc_data, user_id):
    ...
    # Adjust year format if necessary
    if len(yy) == 2:
        yy = '20' + yy
    ...
    if is_ban:
        return f'<code>{cccc}</code>\nStatus: Banned BIN'
    try:
        msg = await stripe_charge1(cc, mm, yy, cvv) 
    except Exception as e:
        return f"<code>{cccc}</code>\nStatus: Error: {str(e)}"

    # Determine card status based on the response
    if any(error_msg in msg for error_msg in [
    ...
    # Return formatted result for the card
    return f"<code>{cccc}</code>\n𝐒𝐭𝐚𝐭𝐮𝐬: {status}\n𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞: {msg}"

# Command handler for /mst command
@Client.on_message(filters.command("mst", prefixes=["/", "."]))
...
'''
    ...
    ccs = message.text.split('mst ', 1)[1].strip() if 'mst ' in message.text else (message.reply_to_message.text.strip() if message.reply_to_message else None)
    ...
    # Calculate the number of cards to process
    ...
    if user_id != OWNER_ID and card_count > 10:
        await message.reply(f"You can process a maximum of 10 cards at a time. You tried to process {card_count} cards.", reply_to_message_id=mid)
        return
    ...
    result_message = "\n<b>- - - - - - - - - - - - - - - - - - - - - - - -</b>\n".join(results)
    result_message += f'''\n\n<b>[<a href="https://t.me/Instuff_bot">ϟ</a>]</b> 𝐆𝐚𝐭𝐞𝐰𝐚𝐲: Stripe Charge $2
