...
from Gates.mau import mau  # Ensure this function is async and non-blocking
...
# Command handler for /mau command
@Client.on_message(filters.command("mau", prefixes=["/", "."]))
async def mst(client, message):
    ...
    ccs = message.text.split('mau ', 1)[1].strip() if 'mau ' in message.text else (message.reply_to_message.text.strip() if message.reply_to_message else None)
    if not ccs:
    ...
    result_message = "\n<b>- - - - - - - - - - - - - - - - - - - - - - - -</b>\n".join(results)
    result_message += f'''\n\n<b>[<a href="https://t.me/Instuff_bot">ϟ</a>]</b> 𝐆𝐚𝐭𝐞𝐰𝐚𝐲: Stripe Auth
