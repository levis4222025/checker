...
from Gates.sc import stripe_charge3
...
import re,time
@Client.on_message(filters.command("sc" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('sc') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('sc ')) > 1 and message.text.split('sc ')[1].strip():
            ccc = message.text.split('sc ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await stripe_charge3(cc,mm,yy,cvv)
