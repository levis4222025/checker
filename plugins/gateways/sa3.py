...
from Gates.sa3 import stripe_auth3
...
import re,time
@Client.on_message(filters.command("sa3" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('sa3') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('sa3 ')) > 1 and message.text.split('sa3 ')[1].strip():
            ccc = message.text.split('sa3 ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await stripe_auth3(cc,mm,yy,cvv)
