...
from Gates.sx import stripe_charge2
...
import re,time
@Client.on_message(filters.command("sx" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('sx') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('sx ')) > 1 and message.text.split('sx ')[1].strip():
            ccc = message.text.split('sx ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await stripe_charge2(cc,mm,yy,cvv)
