...
from Gates.mo1 import Mona_Auth1
...
import re,time
@Client.on_message(filters.command("mo" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('mo') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('mo ')) > 1 and message.text.split('mo ')[1].strip():
            ccc = message.text.split('mo ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await Mona_Auth1(cc,mm,yy,cvv)
    brand , type, level, bank, country, emoji = await bin_data(cc[:6])
    rank = await get_user_rank(user_id)
    end_time = time.perf_counter()
    elapsed_time = f"{end_time - start_time:.2f}"
    if 'Card not approved (483)' in msg or 'Card not approved (487)' in msg or 'Card not approved (489)' in msg or 'Card not approved (490)' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
        msg = f'{msg} | CCN'
    elif 'Card approved (027)' in msg:
         status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
