...
from Gates.b34 import B3_Auth4
...
import re,time
@Client.on_message(filters.command("auth" , prefixes=["/", "."]))
async def chk(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('auth') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('auth ')) > 1 and message.text.split('auth ')[1].strip():
            ccc = message.text.split('auth ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await B3_Auth4(cc,mm,yy,cvv)
    ...
    elif 'Card Issuer Declined CVV' in msg or 'Insufficient Funds' in msg or 'Invalid postal code and cvv' in msg  or 'CVV' in msg or 'avs' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
<b>#Braintree_Auth4 🔥 [/auth]
- - - - - - - - - - - - - - - - - - - - - - - -</b>
