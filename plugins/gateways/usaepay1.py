...
from Gates.usaepay1 import usaepay1
...
import re,time
@Client.on_message(filters.command("ue" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('ue') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('ue ')) > 1 and message.text.split('ue ')[1].strip():
            ccc = message.text.split('ue ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await usaepay1(cc,mm,yy,cvv)
