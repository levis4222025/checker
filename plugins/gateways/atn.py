...
from Gates.authn1 import Auth_net1
...
import re,time
@Client.on_message(filters.command("atn" , prefixes=["/", "."]))
async def atn(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('atn') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('atn ')) > 1 and message.text.split('atn ')[1].strip():
            ccc = message.text.split('atn ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await Auth_net1(cc,mm,yy,cvv)
