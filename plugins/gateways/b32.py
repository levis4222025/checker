...
async def chk(client, message):
    ...
    asyncio.create_task(process_b3_command(client, message))
    
async def process_b3_command(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('b3') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('b3 ')) > 1 and message.text.split('b3 ')[1].strip():
            ccc = message.text.split('b3 ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await B3_Auth2(cc,mm,yy,cvv)
