...
import re,time
@Client.on_message(filters.command("scy" , prefixes=["/", "."]))
async def chk(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('scy') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('scy ')) > 1 and message.text.split('scy ')[1].strip():
            ccc = message.text.split('scy ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await cyber_charge1(cccc)
    brand , type, level, bank, country, emoji = await bin_data(cc[:6])
    rank = await get_user_rank(user_id)
    end_time = time.perf_counter()
    elapsed_time = f"{end_time - start_time:.2f}"
    if 'Charged $1.53 | Code: 200' in msg or 'Invalid Card Verification Number (CVN)' in msg or 'Insufficient funds in the account' in msg or 'card verification number (CVN) did not match' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
<b>#Cybersource ($1.53) 🔥 [/scy]
- - - - - - - - - - - - - - - - - - - - - - - -</b>
