...
from Gates.b3charge2 import Braintree_charge1
...
import re,time
@Client.on_message(filters.command("bc" , prefixes=["/", "."]))
async def b3charge2(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('bc') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('bc ')) > 1 and message.text.split('bc ')[1].strip():
            ccc = message.text.split('bc ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    declined_type, processorCode, processorText, additional, avsPostalCode, avsStreetCode, cvvCode = await Braintree_charge1(cc,mm,yy,cvv)
    if declined_type is None or 'Error' in declined_type:
        return await x.edit('<b>RISK: Retry this BIN later.</b>', disable_web_page_preview=True)
    brand , type, level, bank, country, emoji = await bin_data(cc[:6])
    rank = await get_user_rank(user_id)
    end_time = time.perf_counter()
    elapsed_time = f"{end_time - start_time:.2f}"
    if any(error_msg in processorText for error_msg in [
        'Charged €1',
        ...
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
<b>#Braintree_AVS (€1) 🔥 [/bc]
- - - - - - - - - - - - - - - - - - - - - - - -</b>
<b>[<a href="https://t.me/Instuff_bot">ϟ</a>]</b> 𝐂𝐚𝐫𝐝: <code>{cccc}</code>
<b>[<a href="https://t.me/Instuff_bot">ϟ</a>]</b> 𝐒𝐭𝐚𝐭𝐮𝐬: {status}
<b>[<a href="https://t.me/Instuff_bot">ϟ</a>]</b> 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞: {declined_type} - {processorCode} - {processorText} ({additional})
<b>[<a href="https://t.me/Instuff_bot">ϟ</a>]</b> 𝐀𝐯𝐬 - [{avsStreetCode}] | 𝐂𝐯𝐯 - [{cvvCode}]
<b>- - - - - - - - - - - - - - - - - - - - - - - -</b>
