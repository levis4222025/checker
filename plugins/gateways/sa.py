...
from Gates.sa import stripe_auth1
...
import re,time
@Client.on_message(filters.command("sa" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    # Check if gateway is active using the improved database-based status system
    from plugins.utility.gateway_status import is_gateway_active
    
    # Create a task to check gateway status asynchronously
    is_active = await is_gateway_active('sa')
    if not is_active:
...
'''
    ...
    ccc = None
    if len(message.text.split('sa ')) > 1 and message.text.split('sa ')[1].strip():
            ccc = message.text.split('sa ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await stripe_auth1(cc,mm,yy,cvv)
    ...
    if any(error_msg in msg for error_msg in [
        ...
        'Subcription created successfully',
        'Succeeded',
