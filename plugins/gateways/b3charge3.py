...
from Gates.b3charge3 import b3_charge3
...
import re,time
@Client.on_message(filters.command("bl" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('bl') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('bl ')) > 1 and message.text.split('bl ')[1].strip():
            ccc = message.text.split('bl ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await b3_charge3(cc,mm,yy,cvv)
    brand , type, level, bank, country, emoji = await bin_data(cc[:6])
    rank = await get_user_rank(user_id)
    end_time = time.perf_counter()
    elapsed_time = f"{end_time - start_time:.2f}"
    if 'Charged $5' in msg or 'Card Issuer Declined CVV' in msg or 'Insufficient Funds' in msg or 'Gateway Rejected: avs' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
