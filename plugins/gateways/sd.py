...
from Gates.sd import stripe_charge4
...
import re,time
@Client.on_message(filters.command("sd" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('sd') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('sd ')) > 1 and message.text.split('sd ')[1].strip():
            ccc = message.text.split('sd ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await stripe_charge4(cc,mm,yy,cvv)
