...
'''
    ...
    ccc = None
    if len(message.text.split('vbv ')) > 1 and message.text.split('vbv ')[1].strip():
            ccc = message.text.split('vbv ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await vbv_api(cc)
    brand , type, level, bank, country, emoji = await bin_data(cc[:6])
    rank = await get_user_rank(user_id)
    end_time = time.perf_counter()
    elapsed_time = f"{end_time - start_time:.2f}"
    if 'Authenticate Successful' in msg or 'Authenticate Attempt Successful' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
