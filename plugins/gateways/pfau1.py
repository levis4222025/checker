...
from Gates.pfau1 import payflow_Auth1
...
import re,time
@Client.on_message(filters.command("lt" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('lt') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('lt ')) > 1 and message.text.split('lt ')[1].strip():
            ccc = message.text.split('lt ')[1].strip()
