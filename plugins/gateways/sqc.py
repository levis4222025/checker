...
import re,time
@Client.on_message(filters.command("sqc" , prefixes=["/", "."]))
async def chk(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('sqc') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('sqc ')) > 1 and message.text.split('sqc ')[1].strip():
            ccc = message.text.split('sqc ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await square_key_charge(cc, mm, yy, cvv)
    brand , type, level, bank, country, emoji = await bin_data(cc[:6])
    rank = await get_user_rank(user_id)
    end_time = time.perf_counter()
    elapsed_time = f"{end_time - start_time:.2f}"
    if 'INSUFFICIENT_FUNDS' in msg or 'ADDRESS_VERIFICATION_FAILURE' in msg or 'CVV_FAILURE' in msg or 'TRANSACTION_LIMIT' in msg or 'Charged $0.01' in msg:
        status = '𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅'
    else:
        status = '𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌'
    text = f'''
<b>#Square_Charge ($0.01) 🔥 [/sqc]
- - - - - - - - - - - - - - - - - - - - - - - -</b>
