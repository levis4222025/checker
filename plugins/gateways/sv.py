...
import re,time
@Client.on_message(filters.command("sv" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('sv') == 'off':
...
'''
    ...
    ccc = None
    if len(message.text.split('sv ')) > 1 and message.text.split('sv ')[1].strip():
            ccc = message.text.split('sv ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    session = httpx.AsyncClient(timeout=30, proxies=proxy)
