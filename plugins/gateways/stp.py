...
from Gates.stp import stripe_charge
...
'''
    ...
    ccc = None
    if len(message.text.split('stp ')) > 1 and message.text.split('stp ')[1].strip():
            ccc = message.text.split('stp ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await stripe_charge(cc,mm,yy,cvv)
